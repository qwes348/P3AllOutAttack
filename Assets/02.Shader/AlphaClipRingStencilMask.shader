Shader "Oniboogie/RingStencilMaskTransitionWithID"
{
    Properties
    {
        ...
        _InnerRadius ("Inner Radius", Range(0, 0.5)) = 0.3 // 내부 반지름 조절
        _OuterRadius ("Outer Radius", Range(0, 0.5)) = 1 // 외부 반지름 조절
    }
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            half4 frag (v2f i) : SV_Target
            {
                ...
                // 텍스처 좌표에서 원의 중심으로부터의 거리 계산
                float2 centerdUV = i.uv - 0.5;
                float dist = length(centerdUV);

                
                if (col.a < _AlphaCutoff || dist < _InnerRadius || dist > _OuterRadius) // 알파 부분을 기준으로 클리핑, 원의 바깥쪽 부분 유지, 내부는 투명하게
                {
                    discard;
                }
